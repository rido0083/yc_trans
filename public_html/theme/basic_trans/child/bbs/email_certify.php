...
$row = sql_fetch($sql);
if (!$row['mb_id'])
    alert(_('존재하는 회원이 아닙니다.'), G5_URL);

if ( $row['mb_leave_date'] || $row['mb_intercept_date'] ){
    alert(_('탈퇴 또는 차단된 회원입니다.'), G5_URL);
}
...
if ($mb_md5)
{
    if ($mb_md5 == $row['mb_email_certify2'])
    {
        ...
        alert(_("메일인증 처리를 완료 하였습니다.\\n\\n지금부터 {$mb_id} 아이디로 로그인 가능합니다."), G5_URL);
    }
    else
    {
        alert(_('메일인증 요청 정보가 올바르지 않습니다.'), G5_URL);
    }
}

alert(_('제대로 된 값이 넘어오지 않았습니다.'), G5_URL);
?>