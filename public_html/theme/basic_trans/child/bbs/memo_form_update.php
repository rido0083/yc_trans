...
if ($error_msg && !$is_admin)
    alert(_("회원아이디 '{$error_msg}' 은(는) 존재(또는 정보공개)하지 않는 회원아이디 이거나 탈퇴, 접근차단된 회원아이디 입니다.\\n쪽지를 발송하지 않았습니다."));

if (! count($member_list['id'])){
    alert(_('해당 회원이 존재하지 않습니다.'));
}
    ...
    if (count($member_list['id'])) {
        ...
        if ($point) {
            if ($member['mb_point'] - $point < 0) {
                alert(_('보유하신 포인트(').number_format($member['mb_point'])._('점)가 모자라서 쪽지를 보낼 수 없습니다.'));
            }
        }
    }
}
...
if ($member_list) {
    ...
    alert($str_nick_list._(" 님께 쪽지를 전달하였습니다."), $redirect_url, false);
} else {
    ...
    alert(_("회원아이디 오류 같습니다."), $redirect_url, false);
}
?>