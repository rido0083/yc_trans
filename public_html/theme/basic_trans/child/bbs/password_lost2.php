...
if ($is_member) {
    alert_close(_('이미 로그인중입니다.'), G5_URL);
}
...
if (!$mb['mb_id'] || $mb['mb_leave_date'])
    alert(_('존재하지 않는 회원입니다.'));
else if (is_admin($mb['mb_id']))
    alert(_('관리자 아이디는 접근 불가합니다.'));
...
$subject = _("[".$config['cf_title']."] 요청하신 회원정보 찾기 안내 메일입니다.");
...
$content .= addslashes($mb['mb_name'])." (".addslashes($mb['mb_nick']).")"._(" 회원님은 ".G5_TIME_YMDHIS." 에 회원정보 찾기 요청을 하셨습니다.<br>");
$content .= _('저희 사이트는 관리자라도 회원님의 비밀번호를 알 수 없기 때문에, 비밀번호를 알려드리는 대신 새로운 비밀번호를 생성하여 안내 해드리고 있습니다.<br>');
$content .= _('아래에서 변경될 비밀번호를 확인하신 후, <span style="color:#ff3061"><strong>비밀번호 변경</strong> 링크를 클릭 하십시오.</span><br>')
$content .= _('비밀번호가 변경되었다는 인증 메세지가 출력되면, 홈페이지에서 회원아이디와 변경된 비밀번호를 입력하시고 로그인 하십시오.<br>');
$content .= _('로그인 후에는 정보수정 메뉴에서 새로운 비밀번호로 변경해 주십시오.');
$content .= '</p>';
...
alert_close($email._(' 메일로 회원아이디와 비밀번호를 인증할 수 있는 메일이 발송 되었습니다.\\n\\n메일을 확인하여 주십시오.'));
?>
