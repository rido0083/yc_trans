...
if ($is_admin == 'super') // 최고관리자 통과
    ...
    if ($member['mb_id'] != $group['gr_admin']) // 자신이 관리하는 그룹인가?
        alert(_('자신이 관리하는 그룹의 게시판이 아니므로 삭제할 수 없습니다.'));
    else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
        alert(_('자신의 권한보다 높은 권한의 회원이 작성한 글은 삭제할 수 없습니다.'));
} else if ($is_admin == 'board') { // 게시판관리자이면
    $mb = get_member($write['mb_id']);
    if ($member['mb_id'] != $board['bo_admin']) // 자신이 관리하는 게시판인가?
        alert(_('자신이 관리하는 게시판이 아니므로 삭제할 수 없습니다.'));
    else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
        alert(_('자신의 권한보다 높은 권한의 회원이 작성한 글은 삭제할 수 없습니다.'));
} else if ($member['mb_id']) {
    if ($member['mb_id'] !== $write['mb_id'])
        alert(_('자신의 글이 아니므로 삭제할 수 없습니다.'));
} else {
    if ($write['mb_id'])
        alert(_('로그인 후 삭제하세요.'), G5_BBS_URL.'/login.php?url='.urlencode(get_pretty_url($bo_table, $wr_id)));
    else if (!check_password($wr_password, $write['wr_password']))
        alert(_('비밀번호가 틀리므로 삭제할 수 없습니다.'));
}
...
$row = sql_fetch($sql);
if ($row['cnt'] && !$is_admin)
    alert(_('이 글과 관련된 답변글이 존재하므로 삭제 할 수 없습니다.\\n\\n우선 답변글부터 삭제하여 주십시오.'));
