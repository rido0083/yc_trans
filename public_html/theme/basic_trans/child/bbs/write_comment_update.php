...
if ($w == 'c') // 댓글 입력
{
    ...
    */
    ...
    if ($tmp_point + $board['bo_comment_point'] < 0 && !$is_admin)
        alert(_('보유하신 포인트(').number_format($member['mb_point'])._(')가 없거나 모자라서 댓글쓰기(').number_format($board['bo_comment_point'])._(')가 불가합니다.').'\\n\\n'._('포인트를 적립하신 후 다시 댓글을 써 주십시오.'));
    ...
    if ($comment_id)
    {
        ...
        if (!$reply_array['wr_id'])
            alert(_('답변할 댓글이 없습니다.').'\\n\\n'._('답변하는 동안 댓글이 삭제되었을 수 있습니다.'));

        if($wr['wr_parent'] != $reply_array['wr_parent'])
            alert(_('댓글을 등록할 수 없습니다.'));
...
else if ($w == 'cu') // 댓글 수정
{
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['mb_id'] === $group['gr_admin']) { // 자신이 관리하는 그룹인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                alert(_('그룹관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.'));
        } else
            alert(_('자신이 관리하는 그룹의 게시판이 아니므로 댓글을 수정할 수 없습니다.'));
        ...
        if ($member['mb_id'] === $board['bo_admin']) { // 자신이 관리하는 게시판인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                alert(_('게시판관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.'));
        } else
            alert(_('자신이 관리하는 게시판이 아니므로 댓글을 수정할 수 없습니다.'));
    } else if ($member['mb_id']) {
        if ($member['mb_id'] !== $comment['mb_id'])
            alert(_('자신의 글이 아니므로 수정할 수 없습니다.'));
    } else {
        if( !($comment['mb_id'] === '' && $comment['wr_password'] && check_password($post_wr_password, $comment['wr_password'])) )
            alert(_('댓글을 수정할 권한이 없습니다.'));
    }
    ...
    $row = sql_fetch($sql);
    if ($row['cnt'] && !$is_admin)
        alert(_('이 댓글와 관련된 답변댓글이 존재하므로 수정 할 수 없습니다.'));
