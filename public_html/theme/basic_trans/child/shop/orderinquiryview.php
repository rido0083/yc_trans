...
if (!$is_member) {
    if (get_session('ss_orderview_uid') != $_GET['uid'])
        alert(_("직접 링크로는 주문서 조회가 불가합니다.")."\\n\\n"._("주문조회 화면을 통하여 조회하시기 바랍니다."), G5_SHOP_URL);
}
...
if (!$od['od_id'] || (!$is_member && md5($od['od_id'].$od['od_time'].$od['od_ip']) != get_session('ss_orderview_uid'))) {
    alert(_("조회하실 주문서가 없습니다."), G5_SHOP_URL);
}
...
<!-- 주문상세내역 시작 { -->
<div id="sod_fin">
    <div id="sod_fin_no"><?php echo _('주문번호') ?> <strong><?php echo $od_id; ?></strong></div>
    <section id="sod_fin_list">
        <h2><?php echo _('주문하신 상품') ?></h2>
        ...
        <div class="tbl_head03 tbl_wrap">
            ...
            <thead>
            <tr>
                <th scope="col" rowspan="2"><?php echo _('이미지') ?></th>
                <th scope="col" colspan="7" id="th_itname"><?php echo _('상품명') ?></th>
            </tr>
            <tr class="th_line">
                <th scope="col" id="th_itopt"><?php echo _('옵션명') ?></th>
                <th scope="col" id="th_itqty"><?php echo _('수량') ?></th>
                <th scope="col" id="th_itprice"><?php echo _('소계') ?></th>
                <th scope="col" id="th_itpt"><?php echo _('적립포인트') ?></th>
                <th scope="col" id="th_itsd"><?php echo _('배송비') ?></th>
                <th scope="col" id="th_itst"><?php echo _('상태') ?></th>
            </tr>
            </thead>
            <tbody>
            <?php
        ...
        <div id="sod_sts_wrap">
            <span class="sound_only"><?php echo _('상품 상태 설명') ?></span>
            <button type="button" id="sod_sts_explan_open" class="btn_frmline"><?php echo _('상태설명보기') ?></button>
            <div id="sod_sts_explan">
                <dl id="sod_fin_legend">
                    <dt><?php echo _('주문') ?></dt>
                    <dd><?php echo _('주문이 접수되었습니다.') ?>
                    <dt><?php echo _('입금') ?></dt>
                    <dd><?php echo _('입금(결제)이 완료 되었습니다.') ?>
                    <dt><?php echo _('준비') ?></dt>
                    <dd><?php echo _('상품 준비 중입니다.') ?>
                    <dt><?php echo _('배송') ?></dt>
                    <dd><?php echo _('상품 배송 중입니다.') ?>
                    <dt><?php echo _('완료') ?></dt>
                    <dd><?php echo _('상품 배송이 완료 되었습니다.') ?>
                </dl>
                <button type="button" id="sod_sts_explan_close" class="btn_frmline"><?php echo _('상태설명닫기') ?></button>
            </div>
        </div>
    
    </section>
    <div class="sod_left">
        <h2><?php echo _('결제/배송 정보') ?></h2>
        <?php
        ...
        if($od['od_receipt_price'] > 0)
            ...
            $od_receipt_price = _('아직 입금되지 않았거나 입금정보를 입력하지 못하였습니다.');

        $app_no_subj = '';
        $disp_bank = true;
        $disp_receipt = false;
        if($od['od_settle_case'] == _('신용카드') || $od['od_settle_case'] == 'KAKAOPAY' || is_inicis_order_pay($od['od_settle_case']) ) {
            $app_no_subj = _('승인번호');
            $app_no = $od['od_app_no'];
            $disp_bank = false;
            $disp_receipt = true;
        } else if($od['od_settle_case'] == _('간편결제')) {
            $app_no_subj = '승인번호';
            $app_no = $od['od_app_no'];
            $disp_bank = false;
        } else if($od['od_settle_case'] == _('휴대폰')) {
            $app_no_subj = _('휴대폰번호');
            $app_no = $od['od_bank_account'];
            $disp_bank = false;
            $disp_receipt = true;
        } else if($od['od_settle_case'] == _('가상계좌') || $od['od_settle_case'] == _('계좌이체')) {
            $app_no_subj = _('거래번호');
            $app_no = $od['od_tno'];
        ...
        <section id="sod_fin_pay">
            <h3><?php echo _('결제정보') ?></h3>

            <div class="tbl_head01 tbl_wrap">
                <table>
                <tbody>
                <tr>
                    <th scope="row"><?php echo _('주문번호') ?></th>
                    <td><?php echo $od_id; ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('주문일시') ?></th>
                    <td><?php echo $od['od_time']; ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('결제방식') ?></th>
                    <td><?php echo check_pay_name_replace($od['od_settle_case'], $od, 1); ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('결제금액') ?></th>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('결제일시') ?></th>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('입금자명') ?></th>
                    <td><?php echo get_text($od['od_deposit_name']); ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('입금계좌') ?></th>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('영수증') ?></th>
                    <td>
                        <?php
                        if($od['od_settle_case'] == _('휴대폰'))
                        {
                            if($od['od_pg'] == 'lg') {
                        ...
                        ?>
                        <a href="javascript:;" onclick="<?php echo $hp_receipt_script; ?>"><?php echo _('영수증 출력') ?></a>
                        <?php
                        }

                        if($od['od_settle_case'] == _('신용카드') || is_inicis_order_pay($od['od_settle_case']) || (shop_is_taxsave($od, true) && $misu_price == 0) )
                        {
                            if($od['od_pg'] == 'lg') {
                                require_once G5_SHOP_PATH.'/settle_lg.inc.php';
                                $LGD_TID      = $od['od_tno'];
                        ...
                        ?>
                        <a href="javascript:;" onclick="<?php echo $card_receipt_script; ?>"><?php echo _('영수증 출력') ?></a>
                        <?php
                        }
                        ?>
                    </td>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('포인트사용') ?></th>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('환불 금액') ?></th>
                ...
                if (function_exists('shop_is_taxsave') && shop_is_taxsave($od)) {
                    ...
                    if ($misu_price == 0 && $od['od_receipt_price'] && ($od['od_settle_case'] == _('무통장') || $od['od_settle_case'] == _('계좌이체') || $od['od_settle_case'] == _('가상계좌'))) {
                ?>
                <tr>
                    <th scope="row"><?php echo _('현금영수증') ?></th>
                    <td>
                    <?php
                    if ($od['od_cash'])
                    {
                    ...
                    ?>
                        <a href="javascript:;" onclick="<?php echo $cash_receipt_script; ?>" class="btn_frmline"><?php echo _('현금영수증 확인하기') ?></a>
                    <?php
                    }
                    else
                    {
                    ?>
                        <a href="javascript:;" onclick="window.open('<?php echo G5_SHOP_URL; ?>/taxsave.php?od_id=<?php echo $od_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');" class="btn_frmline"><?php echo _('현금영수증을 발급하시려면 클릭하십시오.') ?></a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
        ...
        <section id="sod_fin_orderer">
            ...
            <div class="tbl_head01 tbl_wrap">
                ...
                <tbody>
                <tr>
                    <th scope="row"><?php echo _('이 름') ?></th>
                    <td><?php echo get_text($od['od_name']); ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('전화번호') ?></th>
                    <td><?php echo get_text($od['od_tel']); ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('핸드폰') ?></th>
                    <td><?php echo get_text($od['od_hp']); ?></td>
                </tr>
                <tr>
                    <th scope="row"><?php echo _('주 소') ?></th>
        ...
        <section id="sod_fin_receiver">
            ...
            <div class="tbl_head01 tbl_wrap">
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('희망배송일') ?></th>
                ...
                ?>
                <tr>
                    <th scope="row"><?php echo _('전하실 말씀') ?></th>
        ...
        <section id="sod_fin_dvr">
            ...
            <div class="tbl_head01 tbl_wrap">
                ...
                ?>
                <tr>
                    <td class="empty_table"><?php echo _('아직 배송하지 않았거나 배송정보를 입력하지 못하였습니다.') ?></td>
                </tr>
                <?php
                }
                ?>
    ...
    <div class="sod_right">
        <ul id="sod_bsk_tot" class="order_view_infos">
            <li class="sod_bsk_dvr">
                <span><?php echo _('주문총액') ?></span>
                <strong><?php echo number_format($od['od_cart_price']); ?> <?php echo _('원') ?></strong>
            </li>
            <?php if($od['od_cart_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span><?php echo _('개별상품 쿠폰할인') ?></span>
                <strong><?php echo number_format($od['od_cart_coupon']); ?> <?php echo _('원') ?></strong>
                
            </li>
            <?php } ?>
            <?php if($od['od_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span><?php echo _('주문금액 쿠폰할인') ?></span>
                <strong><?php echo number_format($od['od_coupon']); ?> <?php echo _('원') ?></strong>
            ...
            <li class="sod_bsk_dvr">
                ...
                <strong><?php echo number_format($od['od_send_cost']); ?> <?php echo _('원') ?></strong>
                
            </li>
            <?php } ?>
            <?php if($od['od_send_coupon'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span><?php echo _('배송비 쿠폰할인') ?></span>
                <strong><?php echo number_format($od['od_send_coupon']); ?> <?php echo _('원') ?></strong>
                
            </li>
            <?php } ?>
            <?php if ($od['od_send_cost2'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span><?php echo _('추가배송비') ?></span>
                <strong><?php echo number_format($od['od_send_cost2']); ?> <?php echo _('원') ?></strong>
            </li>
            <?php } ?>
            <?php if ($od['od_cancel_price'] > 0) { ?>
            <li class="sod_bsk_dvr">
                <span><?php echo _('취소금액') ?></span>
                <strong><?php echo number_format($od['od_cancel_price']); ?> <?php echo _('원') ?></strong>
                
            </li>
            <?php } ?>
            <li class="sod_bsk_cnt">
                <span><?php echo _('총계') ?></span>
                <strong><?php echo number_format($tot_price); ?> <?php echo _('원') ?></strong>
            </li>
            <li class="sod_bsk_point">
                <span><?php echo _('적립포인트') ?></span>
                <strong><?php echo number_format($tot_point); ?> <?php echo _('점') ?></strong>
            </li>
        </ul>
        
        <section id="sod_fin_tot">
            <h2><?php echo _('결제합계') ?></h2>
            ...
            <ul>
                ...
                <li id="alrdy">
                    ...
                    <div class="right">
                        <p><span class="title"><i class="fa fa-angle-right" aria-hidden="true"></i> <?php echo _('포인트 결제') ?></span><?php echo number_format($od['od_receipt_point']); ?> <?php echo _('점') ?></p>
                        <p><span class="title"><i class="fa fa-angle-right" aria-hidden="true"></i> <?php echo _('실결제') ?></span><?php echo number_format($od['od_receipt_price']); ?> <?php echo _('원') ?></p>
                    </div>
                    <?php } ?>
                </li>
            </ul>
        ...
        <section id="sod_fin_cancel">
            <h2><?php echo _('주문취소') ?></h2>
            <?php
            // 취소한 내역이 없다면
            if ($cancel_price == 0) {
                if ($custom_cancel) {
            ?>
            <button type="button" onclick="document.getElementById('sod_fin_cancelfrm').style.display='block';"><?php echo _('주문 취소하기') ?></button>
            ...
            <div id="sod_fin_cancelfrm">
                ...
                <label for="cancel_memo" class="sound_only"><?php echo _('취소사유') ?></label>
                <input type="text" name="cancel_memo" id="cancel_memo" required class="frm_input required" size="40" maxlength="100" placeholder="<?php echo _('취소사유') ?>">
            ...
            ?>
            <p><?php echo _('주문 취소, 반품, 품절된 내역이 있습니다.') ?></p>
            <?php } ?>
        </section>
    </div>
    ...
    <?php if ($od['od_settle_case'] == _('가상계좌') && $od['od_misu'] > 0 && $default['de_card_test'] && $is_admin && $od['od_pg'] == 'kcp') {
    preg_match("/\s{1}([^\s]+)\s?/", $od['od_bank_account'], $matchs);
    $deposit_no = trim($matchs[1]);
    ?>
    <p><?php echo _('관리자가 가상계좌 테스트를 한 경우에만 보입니다.') ?></p>
    <div class="tbl_frm01 tbl_wrap">
        <form method="post" action="http://devadmin.kcp.co.kr/Modules/Noti/TEST_Vcnt_Noti_Proc.jsp" target="_blank">
        <table>
        <caption><?php echo _('모의입금처리') ?></caption>
        <colgroup>
            <col class="grid_3">
            <col>
        </colgroup>
        <tbody>
        <tr>
            <th scope="col"><label for="e_trade_no"><?php echo _('KCP 거래번호') ?></label></th>
            <td><input type="text" name="e_trade_no" value="<?php echo $od['od_tno']; ?>"></td>
        </tr>
        <tr>
            <th scope="col"><label for="deposit_no"><?php echo _('입금계좌') ?></label></th>
            <td><input type="text" name="deposit_no" value="<?php echo $deposit_no; ?>"></td>
        </tr>
        <tr>
            <th scope="col"><label for="req_name"><?php echo _('입금자명') ?></label></th>
            <td><input type="text" name="req_name" value="<?php echo $od['od_deposit_name']; ?>"></td>
        </tr>
        <tr>
            <th scope="col"><label for="noti_url"><?php echo _('입금통보 URL') ?></label></th>
...
$(function() {
    $("#sod_sts_explan_open").on("click", function() {
        ...
        if($explan.is(":visible")) {
            $explan.slideUp(200);
            $("#sod_sts_explan_open").text("<?php echo _('상태설명보기') ?>");
        } else {
            $explan.slideDown(200);
            $("#sod_sts_explan_open").text("<?php echo _('상태설명닫기') ?>");
        }
    });
...
{
    if(!confirm("<?php echo _('주문을 정말 취소하시겠습니까?') ?>"))
        return false;

    var memo = f.cancel_memo.value;
    if(memo == "") {
        alert("<?php echo _('취소사유를 입력해 주십시오.') ?>");
        return false;
    }
