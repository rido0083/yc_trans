...
if (!$is_member)
    alert(_('회원 전용 서비스 입니다.'), G5_BBS_URL.'/login.php?url='.urlencode($url));
...
if ($w == "d")
{
    ...
    if($row['mb_id'] != $member['mb_id'])
        alert(_('위시리시트 상품을 삭제할 권한이 없습니다.'));
...
else
{
    ...
    if(!$it_id)
        alert(_('상품코드가 올바르지 않습니다.'), G5_SHOP_URL);
    ...
    if(!$row['it_id'])
        alert(_('상품정보가 존재하지 않습니다.'), G5_SHOP_URL);
