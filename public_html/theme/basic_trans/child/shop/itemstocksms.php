...
if(!$it['it_soldout'] || !$it['it_stock_sms'])
    alert_close(_('재입고SMS 알림을 신청할 수 없는 상품입니다.'));
...
<div id="sit_sms_new" class="new_win">
    ...
    <div class="form_01">
        <ul>
            ...
            <li>
                <label for="ss_hp" class="sound_only"><?php echo _('휴대폰번호') ?><strong> <?php echo _('필수') ?></strong></label>
            ...
            <li>
                <strong><?php echo _('개인정보처리방침안내') ?></strong>
        ...
        <div id="sms_agree" class="chk_box">
            ...
            <label for="agree"><span></span><?php echo _('개인정보처리방침안내의 내용에 동의합니다.') ?></label>
        </div>
        
        <div class="win_btn">
            <button type="submit" class="btn_submit"><?php echo _('확인') ?></button>
            <button type="button" onclick="window.close();" class="btn_close"><?php echo _('닫기') ?></button>
        </div>
    </div>
    </form>
</div>
...
{
    if(!f.agree.checked) {
        alert("<?php echo _('개인정보처리방침안내에 동의해 주십시오.') ?>");
        return false;
    }

    if(confirm("<?php echo _('재입고SMS 알림 요청을 등록하시겠습니까?') ?>")) {
        return true;
    } else {
        window.close();
        return false;
