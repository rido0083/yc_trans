...
if($od['od_cancel_price'] > 0 || $ct['od_count1'] != $ct['od_count2']) {
    alert(_("취소할 수 있는 주문이 아닙니다."), G5_SHOP_URL."/orderinquiryview.php?od_id=$od_id&amp;uid=$uid");
}
...
if($od['od_tno']) {
    switch($od['od_pg']) {
        ...
        case 'inicis':
            ...
            $cancel_msg = iconv_euckr(_('주문자 본인 취소-').$cancel_memo);
            ...
            if($res_cd != '00') {
                alert(iconv_utf8($res_msg)._(' 코드 : )'.$res_cd);
            }
            break;
        default:
            ...
            if($od['od_escrow']) {
                ...
                $_POST['mod_type'] = 'STE2';
                if($od['od_settle_case'] == _('가상계좌'))
                    $_POST['mod_type'] = 'STE5';
            }
            $_POST['mod_desc'] = iconv("utf-8", "euc-kr", _('주문자 본인 취소-').$cancel_memo);
...
// 장바구니 자료 취소
sql_query(" update {$g5['g5_shop_cart_table']} set ct_status = "._('취소')." where od_id = '$od_id' ");
...
$sql = " update {$g5['g5_shop_order_table']}
            set od_send_cost = '0',
                ...
                od_send_coupon = '0',
                od_status = '취소',
                od_shop_memo = concat(od_shop_memo,\"\\n"._("주문자 본인 직접 취소 - ").G5_TIME_YMDHIS._(" (취소이유 :").{$cancel_memo})\")
            where od_id = '$od_id' ";
sql_query($sql);

// 주문취소 회원의 포인트를 되돌려 줌
if ($od['od_receipt_point'] > 0)
    insert_point($member['mb_id'], $od['od_receipt_point'],_("주문번호")." $od_id"._("본인 취소"));
