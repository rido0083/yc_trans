...
<section class="couponzone_list" id="point_coupon">
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$coupon .= '<div class="coupon_date"><span class="sound_only">'._('기한').'</span>'._('다운로드 후 ').number_format($row['cz_period'])._('일').'</div>'.PHP_EOL;
		$coupon .= '<div class="coupon_btn"><button type="button" class="coupon_download btn02'.$disabled.'" data-cid="'.$row['cz_id'].'">'._('포인트 ').number_format($row['cz_point'])._('점 차감').'</button></div>'.PHP_EOL;
