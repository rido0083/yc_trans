...
<div id="scp_list" class="new_win">
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if($row['cp_type'])
            ...
            $cp_price = '<strong>'.number_format($row['cp_price']).'</strong> '._('원');
    ...
    if(!$cp_count)
        echo '<li class="empty_list">'._('사용할 수 있는 쿠폰이 없습니다.').'</li>';
    ?>
    </ul>

    <div class="win_btn"><button type="button" onclick="window.close();" class="btn_close"><?php echo _('창닫기')?></button></div>
</div>
