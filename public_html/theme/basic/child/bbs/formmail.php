...
if (!$config['cf_email_use'])
    alert_close(_('환경설정에서 \"메일발송 사용\"에 체크하셔야 메일을 발송할 수 있습니다.\\n\\n관리자에게 문의하시기 바랍니다.'));
...
if ($is_member && !$member['mb_open'] && $is_admin != "super" && $member['mb_id'] != $mb_id)
    alert_close(_('자신의 정보를 공개하지 않으면 다른분에게 메일을 보낼 수 없습니다.\\n\\n정보공개 설정은 회원정보수정에서 하실 수 있습니다.'));

if ($mb_id)
{
    $mb = get_member($mb_id);
    if (!$mb['mb_id'])
        alert_close(_('회원정보가 존재하지 않습니다.\\n\\n탈퇴한 회원일 수 있습니다.'));

    if (!$mb['mb_open'] && $is_admin != "super")
        alert_close(_('정보공개를 하지 않았습니다.'));
}
