...
if (!$po['po_id'])
    alert(_('po_id 값이 제대로 넘어오지 않았습니다.)');

if ($member['mb_level'] < $po['po_level'])
    alert_close(_('권한 ').$po['po_level']._(' 이상 회원만 투표에 참여하실 수 있습니다.'));

$gb_poll = preg_replace('/[^0-9]/', '', $gb_poll);
if(!$gb_poll)
    alert_close(_('항목을 선택하세요.'));
...
if (!($search_ip || $search_mb_id)) {
    ...
    alert($po['po_subject']._('에 이미 참여하셨습니다.'), $result_url);
}
