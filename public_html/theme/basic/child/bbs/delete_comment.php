...
if (!$write['wr_id'] || !$write['wr_is_comment'])
    alert(_('등록된 코멘트가 없거나 코멘트 글이 아닙니다.'));
...
if ($is_admin == 'super') // 최고관리자 통과
    ...
    if ($member['mb_id'] === $group['gr_admin']) { // 자신이 관리하는 그룹인가?
        if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            ...
            alert(_('그룹관리자의 권한보다 높은 회원의 코멘트이므로 삭제할 수 없습니다.'));
    } else
        alert(_('자신이 관리하는 그룹의 게시판이 아니므로 코멘트를 삭제할 수 없습니다.'));
    ...
    if ($member['mb_id'] === $board['bo_admin']) { // 자신이 관리하는 게시판인가?
        if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            ...
            alert(_('게시판관리자의 권한보다 높은 회원의 코멘트이므로 삭제할 수 없습니다.'));
    } else
        alert(_('자신이 관리하는 게시판이 아니므로 코멘트를 삭제할 수 없습니다.'));
...
$row = sql_fetch($sql);
if ($row['cnt'] && !$is_admin)
    alert(_('이 코멘트와 관련된 답변코멘트가 존재하므로 삭제 할 수 없습니다.'));
