...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            $msg = _("비회원은 이 게시판에 접근할 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.");
        ...
        if ($is_admin == "super" || $is_admin == "group") {
            ...
            $row = sql_fetch($sql);
            if (!$row['cnt']) {
                alert(_("접근 권한이 없으므로 글읽기가 불가합니다.\\n\\n궁금하신 사항은 관리자에게 문의 바랍니다."), G5_URL);
            }
        }
    }
    ...
    if ($config['cf_cert_use'] && !$is_admin) {
        ...
        if ($board['bo_use_cert'] != '' && $is_guest) {
            alert(_('이 게시판은 본인확인 하신 회원님만 글읽기가 가능합니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.'), G5_BBS_URL.'/login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(get_pretty_url($bo_table, $wr_id, $qstr)));
        }
    ...
    if (!get_session($ss_name))
    {
        ...
        if ($write['mb_id'] && $write['mb_id'] === $member['mb_id']) {
            ...
            if ($config['cf_use_point'] && $board['bo_read_point'] && $member['mb_point'] + $board['bo_read_point'] < 0)
                alert(_('보유하신 포인트').'('.number_format($member['mb_point']).')'._('가 없거나 모자라서 글읽기').'('.number_format($board['bo_read_point']).')'._('가 불가합니다.\\n\\n포인트를 모으신 후 다시 글읽기 해 주십시오.'));
...
include_once(PR_BBS_PATH.'/board_head.php');

// 게시물 아이디가 있다면 게시물 보기를 INCLUDE
if (isset($wr_id) && $wr_id) {
    //include_once(G5_BBS_PATH.'/view.php');
    include_once(PR_BBS_PATH.'/view.php');
}
...
if ($member['mb_level'] >= $board['bo_list_level'] && $board['bo_use_list_view'] || empty($wr_id))
    ...
    include_once (PR_BBS_PATH.'/list.php');

// include_once(G5_BBS_PATH.'/board_tail.php');
include_once(PR_BBS_PATH.'/board_tail.php');
