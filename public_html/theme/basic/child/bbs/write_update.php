...
if ($w == '' || $w == 'r') {
    ...
    if (get_session('ss_bo_table') != $_POST['bo_table'] || get_session('ss_wr_id') != $_POST['wr_id']) {
        alert(_('올바른 방법으로 수정하여 주십시오.'), get_pretty_url($bo_table));
    }
    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if ($member['mb_id'] != $group['gr_admin']) // 자신이 관리하는 그룹인가?
            alert(_('자신이 관리하는 그룹의 게시판이 아니므로 수정할 수 없습니다.'), $return_url);
        else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            alert(_('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.'), $return_url);
    } else if ($is_admin == 'board') { // 게시판관리자이면
        $mb = get_member($write['mb_id']);
        if ($member['mb_id'] != $board['bo_admin']) // 자신이 관리하는 게시판인가?
            alert(_('자신이 관리하는 게시판이 아니므로 수정할 수 없습니다.'), $return_url);
        else if ($member['mb_level'] < $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
            alert(_('자신의 권한보다 높은 권한의 회원이 작성한 글은 수정할 수 없습니다.'), $return_url);
    } else if ($member['mb_id']) {
        if ($member['mb_id'] != $write['mb_id'])
            alert(_('자신의 글이 아니므로 수정할 수 없습니다.'), $return_url);
    } else {
        if ($write['mb_id'])
            alert(_('로그인 후 수정하세요.'), G5_BBS_URL.'/login.php?url='.urlencode($return_url));
    }
    ...
    if ($member['mb_id']) {
        ...
        $mb_id = "";
        // 비회원의 경우 이름이 누락되는 경우가 있음
        if (!trim($wr_name)) alert(_("이름은 필히 입력하셔야 합니다."));
        $wr_name = clean_xss_tags(trim($_POST['wr_name']));
        $wr_email = get_email_address(trim($_POST['wr_email']));
    }
...
if($w == 'u') {
    ...
    if($file_count && (int)$file['count'] > $board['bo_upload_count'])
        alert(_('기존 파일을 삭제하신 후 첨부파일을 ').number_format($board['bo_upload_count'])._('개 이하로 업로드 해주십시오.'));
} else {
    if($file_count > $board['bo_upload_count'])
        alert(_('첨부파일을 ').number_format($board['bo_upload_count'])._('개 이하로 업로드 해주십시오.'));
}
    ...
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (is_uploaded_file($tmp_file)) {
            ...
            if (!$is_admin && $filesize > $board['bo_upload_size']) {
                $file_upload_msg .= '\"'.$filename._('\" 파일의 용량(').number_format($filesize)._(' 바이트)이 게시판에 설정(').number_format($board['bo_upload_size'])._(' 바이트)된 값보다 크므로 업로드 하지 않습니다.').'\\n';
                continue;
            }
...
if (!($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
    ...
    $warr = array( ''=>_('입력'), 'u'=>_('수정'), 'r'=>_('답변'), 'c'=>_('코멘트'), 'cu'=>_('코멘트 수정') );
    $str = $warr[$w];

    $subject = '['.$config['cf_title'].'] '.$board['bo_subject']._(' 게시판에 ').$str._('글이 올라왔습니다.');
