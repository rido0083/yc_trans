...
if(isset($_POST['js']) && $_POST['js'] === "on") {
    ...
    if (!($bo_table && $wr_id)) {
        $error = _('값이 제대로 넘어오지 않았습니다.');
        print_result($error, $count);
    }

    $ss_name = 'ss_view_'.$bo_table.'_'.$wr_id;
    if (!get_session($ss_name)) {
        $error = _('해당 게시물에서만 추천 또는 비추천 하실 수 있습니다.');
        print_result($error, $count);
    }

    $row = sql_fetch(" select count(*) as cnt from {$g5['write_prefix']}{$bo_table} ", FALSE);
    if (!$row['cnt']) {
        $error = _('존재하는 게시판이 아닙니다.');
        print_result($error, $count);
    }
    ...
    if ($good == 'good' || $good == 'nogood')
    {
        if($write['mb_id'] == $member['mb_id']) {
            $error = _('자신의 글에는 추천 또는 비추천 하실 수 없습니다.');
            print_result($error, $count);
        }

        if (!$board['bo_use_good'] && $good == 'good') {
            $error = _('이 게시판은 추천 기능을 사용하지 않습니다.');
            print_result($error, $count);
        }

        if (!$board['bo_use_nogood'] && $good == 'nogood') {
            $error = _('이 게시판은 비추천 기능을 사용하지 않습니다.');
            print_result($error, $count);
        }
    ...
    if (!$is_member)
    {
        ...
        alert(_('회원만 가능합니다.'), $href);
    }

    if (!($bo_table && $wr_id))
        alert(_('값이 제대로 넘어오지 않았습니다.'));

    $ss_name = 'ss_view_'.$bo_table.'_'.$wr_id;
    if (!get_session($ss_name))
        alert(_('해당 게시물에서만 추천 또는 비추천 하실 수 있습니다.'));

    $row = sql_fetch(" select count(*) as cnt from {$g5['write_prefix']}{$bo_table} ", FALSE);
    if (!$row['cnt'])
        alert(_('존재하는 게시판이 아닙니다.'));

    if ($good == 'good' || $good == 'nogood')
    {
        if($write['mb_id'] == $member['mb_id'])
            alert(_('자신의 글에는 추천 또는 비추천 하실 수 없습니다.'));

        if (!$board['bo_use_good'] && $good == 'good')
            alert(_('이 게시판은 추천 기능을 사용하지 않습니다.'));

        if (!$board['bo_use_nogood'] && $good == 'nogood')
            alert(_('이 게시판은 비추천 기능을 사용하지 않습니다.'));
        ...
        else
        {
            ...
            alert(_("이 글을 $status 하셨습니다."), '', false);
        }
    }
}
