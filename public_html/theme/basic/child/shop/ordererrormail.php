...
$subject = $config['cf_title']._(' 주문 오류 알림 메일');

if($error == 'order') {
    $content = '<p>'._('주문정보를 DB에 입력하는 중 오류가 발생했습니다.').'</p>';
} else if($error == 'status') {
    $content = '<p>'._('주문 상품의 상태를 변경하는 중 DB 오류가 발생했습니다.').'</p>';
}

if($tno) {
    $content .= '<p>'._('PG사의 ').$od_settle_case._('는 자동 취소되었습니다.').'</p>';
