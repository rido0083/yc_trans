...
function itemuse_page($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" class="pg_page pg_prev">'._('이전').'</a>'.PHP_EOL;
    ...
    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" class="pg_page pg_next">'._('다음').'</a>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<a href="'.$url.$total_page.$add.'" class="pg_page pg_end">'_('맨끝').'</a>'.PHP_EOL;
    }
